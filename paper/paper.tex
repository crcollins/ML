...
\begin{abstract}
By using machine learning methods, the HOMO, LUMO, and band gap can be predicted to within 0.09 eV, 0.08 eV, and 0.10 eV respectively. These results also come with the added benefit of being 30,000,000 times faster than the equivilant Density Functional Theory calculation.
\end{abstract}

% \doublespace
\section{Introduction}

There are many possible molecules that could be used as organic solar cells, the problem is current organic solar cells have poor effeciency [CITE]. Computational
...
For this study, we will be looking at using different machine learning methods in conjunction with Density Functional Theory calculations to be able to predict the


\section{Naming Scheme}

To be able to represent these molecular structures to the machine learning algorthims, a common naming scheme was formed such that it could be reaily converted into a vector form. This naming scheme exploits the planar nature of benzobisazoles as well as their cruxiformic structure.
The naming scheme is broken down into three separate components. These parts are the core, the sides, and the options.

\begin{figure}[H]
  \begin{center}
    \includegraphics[width=1.25in]{figures/TXY.png}
  \end{center}
  \caption{TXY Core}
  \label{fig:txy}
\end{figure}
...
\begin{figure}[H]
  \begin{center}
    \includegraphics[width=1.75in]{figures/aryl.png}
  \end{center}
  \caption{Aryl Groups}
\end{figure}
...
To be able to represent these structures to the computer, and consequently to the machine learning algorthms they must be vectorized.
...
In previous work \cite{montavon_machine_2013}, it has been shown that different machine learning algorthms have had drastically different performance when used to predict molecular properties. Because of this, seven algorthms were picked to compare. These were mean value, linear regression, linear ridged regression, tree regression, k-nearest neighbors, and support vector machines. The mean value method is used as a baseline for all of the other methods. ()
...
Roughly 1100 benzobisazole structures were built. Once they were built the structures were optimized using Density Functional Theory (DFT) with B3LYP/6-31g*. After the optimization, the molecules were then ran with Time Dependent Density Functional Theory (TDDFT) with B3LYP/6-31g*.
...
\end{table}
 ...
 \begin{figure}[H]
   \begin{center}
     \includegraphics[width=5in]{figures/gapbars.pdf}
   \end{center}
   \caption{Band Gap Results}
   \label{fig:scan}
 \end{figure}
...
\end{table}
 ...
 \begin{figure}[H]
   \begin{center}
     \includegraphics[width=5in]{figures/homobars.pdf}
   \end{center}
   \caption{HOMO Results}
   \label{fig:scan}
 \end{figure}
...
\end{table}
 ...
 \begin{figure}[H]
   \begin{center}
     \includegraphics[width=5in]{figures/lumobars.pdf}
   \end{center}
   \caption{LUMO Results}
   \label{fig:scan}
 \end{figure}
...
Overall, the support vector machines with Gaussian kernel had the best performance across all of the goal features and all of the feature vectors. The optimized paramters for the method were $C \approx 10$ and $\gamma \approx 0.05$.
...
Unexpectedly, the decay function with distance corrections did not perform any better than the regular decay feature vector. In theory, this should have worked better with hetrogenous side chains, but its use seemed to only increase the MAE.
...
To run the original DFT and TDDFT calculations for these moelcules took about 35,0000 CPU hours. Calculating the same properties using these machine learning methods took about 4 seconds.

% In addition to the machine learning methods used in this study, there was also an attempt to use Neural Networks for the predictions. Unfortunatly, the results were not satisfactory relative to the amount of time required (0.20 eV MAE). Drastic changes to the network made minimal differences to the final error. This is attirbuted to inexperience with proper methodologies with Neural Networks.

\section{Future Work}

Continue making tweaks to reduce error
Collect more data
...
Test with additional properties (total energy, dipole, etc.)
Generalize for any structure
Make it faster
% In addition to the machine learning methods used in this study, there was also an attempt to use Neural Networks for the predictions. Unfortunatly, the results were not satisfactory relative to the amount of time required (0.20 eV MAE). Drastic changes to the network made minimal differences to the final error. This is attirbuted to inexperience with proper methodologies with Neural Networks.
